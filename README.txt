...
Leads and the administrator can move records to Trash. Only the administrator can restore or permanently delete them. Permanent deletion requires confirmation followed by typing the exact name; it removes the record, its pings, links and saved versions, and source application rows not shared by another record. Google Sheets version history and external copies are outside the portal deletion. Individual LWServers resets start in Record management. Leads and admin can fetch an exact-ID snapshot, choose whether to retain messages and notes, and submit a request. Only admin can approve in Data sync > Individual resets, with a second typed-name confirmation. Reset returns the record to Contact and clears application/dashboard details, aliases, previous names, account links, assignments, reminders and old activity. Source rows are removed when not shared by other records; a reset audit entry remains. General contacts remain excluded from routine bulk sync. Prospects search spans the full active directory; adding a prospect requires searching existing players first. Reminders and ping sounds work while the app is open; no email or background push service is configured. Some descriptive interface text still falls back to English.
