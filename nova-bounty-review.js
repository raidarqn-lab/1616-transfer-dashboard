...
async function showDirectoryProfile(profile){
 profileCache.set(profile.key,profile);const out=$('directory-profile'),heading=el('h3',profile.name||profile.translatedName||'Player profile'),card=profileCard(profile);out.replaceChildren(heading,card);if(out.showModal){const close=el('button','Close profile');close.className='profile-close';close.onclick=()=>out.close();out.prepend(close);if(!out.open)out.showModal();}
 ...
 for(const [key,label] of choices){const button=el('button',label);button.type='button';button.dataset.key=key;button.setAttribute('role','tab');button.onclick=()=>draw(key);tabs.append(button);}draw('all');out.scrollIntoView({behavior:'smooth',block:'start'});
...
function drawRoster(players,results){
 ...
 const count=rosterMetric==='vs'?6:7,minimum=rosterMetric==='vs'?7200000:5000;results.append(el('p',`Daily target: ${formatScore(minimum)} · Green: at or above target · Red: below target · —: not confirmed. Columns use game dates; uploads open two days later.`));
 const table=el('table');table.className='r4-table contacts-table roster-grid';const thead=el('thead'),head=el('tr');head.append(el('th','Player'),el('th','Rank'));for(let i=0;i<count;i++){const d=new Date(rosterWeek+'T00:00:00Z');d.setUTCDate(d.getUTCDate()+i);const th=el('th',['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][i]);th.append(el('small',d.toISOString().slice(5)));head.append(th);}head.append(el('th','Weekly total'));thead.append(head);table.append(thead);const tbody=el('tbody');
