...
function editPlayerOverview(body,profile,full,onSaved){
 ...
 const fields=[['name','Player name'],['translatedName','Translated name'],['alliance','Alliance'],['server','Server'],['allianceRank','Alliance rank'],['power','Hero power (M)'],['profession','Profession level'],['kills','Kills']];const inputs=new Map();
 for(const [key,label] of fields){const input=el(key==='allianceRank'?'select':'input');input.setAttribute('aria-label',label);input.setAttribute('form',form.id);if(key==='allianceRank'){for(const rank of ['','R1','R2','R3','R4','R5']){const option=el('option',rank||'Not recorded');option.value=rank;input.append(option);}}else{input.type=['power','profession','kills'].includes(key)?'number':'text';input.maxLength=160;if(input.type==='number'){input.min='0';input.max='1000000000000000';input.step=key==='power'?'any':'1';}}input.value=full.details?.[key]??'';input.required=['name','server'].includes(key);if(key==='server')input.pattern='[0-9]{1,6}';const pair=Array.from(body.querySelectorAll('.player-info-grid dl>div')).find(node=>node.querySelector('dt')?.textContent===label);if(pair)pair.querySelector('dd').replaceChildren(input);inputs.set(key,input);}
...
function drawPlayerHR(body,profile,full){
 ...
 for(const record of records){const card=el('section');card.className='player-info-card hr-entry';card.dataset.status=record.status;const heading=el('div');heading.className='hr-entry-heading';const badge=el('span',overdue(record)?'Overdue':record.status);badge.className='hr-badge'+(overdue(record)?' hr-overdue':'');heading.append(el('h3',record.title),badge);card.append(heading,el('small',`${record.category} · Event: ${record.event_date}`));const notes=el('p',record.details);notes.style.whiteSpace='pre-wrap';card.append(notes,el('p',`Added by ${record.author_name} · ${new Date(record.created_at).toLocaleString()}`),el('small',`Verified account: ${record.author_email}`));if(record.follow_up)card.append(el('p',`Follow-up: ${record.follow_up}`));
...
async function showDirectoryProfile(profile){
 ...
 try{const full=await call({action:'profile',playerKey:profile.key,includeActivity:true});if(!heading.isConnected)return;Object.assign(profile,full.details||{});heading.textContent=profile.name||'Player profile';setPlayerPhoto(avatar,full.avatarUrl,profile.name);const choices=[['all','Overview'],['vs','Alliance Duel'],['donations','Alliance Donations'],['desert_storm','Desert Storm'],['canyon_storm','Canyon Storm'],['bounties','Bounties'],['hr','Human Resources']];
 ...
 const draw=key=>{body.classList.remove('hr-workspace');body.replaceChildren();for(const button of tabs.children)button.setAttribute('aria-selected',String(button.dataset.key===key));const name=choices.find(c=>c[0]===key)[1];body.append(el('h2',name));
 ...
 for(const [key,label] of choices){const button=el('button',label);button.type='button';button.dataset.key=key;button.setAttribute('role','tab');button.onclick=()=>draw(key);tabs.append(button);}draw('all');
...
function drawDuelStrategy(results,week){
 const box=el('section');box.className='duel-strategy';box.setAttribute('aria-label','Weekly Alliance Duel strategy');box.append(el('p','Loading weekly strategy…'));results.append(box);
 let strategy;
 const labels={unset:'Strategy not set',hold:'Hold / Save week',win:'Win week'};
 const descriptions={unset:'Leadership has not selected a strategy for this week.',hold:'Aim for 7,200,000 each day to open your boxes. Save remaining resources.',win:'Aim for the daily minimum, then spend conservatively as needed to win against the opponent. Follow leadership’s direction.'};
 const render=()=>{box.replaceChildren();box.dataset.mode=strategy.mode;const heading=el('strong',labels[strategy.mode]);heading.className='duel-strategy-flag';box.append(heading,el('p',descriptions[strategy.mode]));if(strategy.note)box.append(el('p',strategy.note));if(strategy.updatedAt)box.append(el('small',`Updated ${new Date(strategy.updatedAt).toLocaleString()} · ${strategy.updatedBy}`));const edit=el('button','Edit weekly strategy');edit.type='button';edit.onclick=editStrategy;box.append(edit);};
 const editStrategy=()=>{box.replaceChildren();const form=el('form'),mode=el('select'),note=el('textarea'),status=el('p');mode.setAttribute('aria-label','Weekly strategy');for(const [value,label] of Object.entries(labels)){const option=el('option',label);option.value=value;mode.append(option);}mode.value=strategy.mode;note.setAttribute('aria-label','Leadership note');note.placeholder='Opponent, priorities, or instructions for the week';note.maxLength=1000;note.value=strategy.note;const review=el('button','Review strategy'),cancel=el('button','Cancel');cancel.type='button';cancel.onclick=render;form.append(el('strong',`Week of ${week}`),mode,note,review,cancel,status);box.append(form);form.onsubmit=event=>{event.preventDefault();review.disabled=true;mode.disabled=true;note.disabled=true;const confirm=el('button','Confirm weekly strategy'),back=el('button','Back to editing');confirm.type=back.type='button';status.textContent=`Save ${labels[mode.value]} for the week of ${week}?`;back.onclick=()=>{confirm.remove();back.remove();review.disabled=mode.disabled=note.disabled=false;status.textContent='';};confirm.onclick=async()=>{confirm.disabled=back.disabled=cancel.disabled=true;try{strategy=await call({action:'profile',profileOperation:'week-save',week,mode:mode.value,note:note.value,expectedRevision:strategy.revision,confirmed:true});render();}catch(error){status.textContent='Not saved. Reload the roster if another leader changed this week. '+error.message;confirm.disabled=back.disabled=cancel.disabled=false;}};form.append(confirm,back);};};
