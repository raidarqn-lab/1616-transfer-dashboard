...
async function showDirectoryProfile(profile){
 ...
 if(key==='bounties'){
  ...
  if(!history.length){empty('No bounty history yet','Submissions and bounty evidence linked to this player will appear here.');return;}
  for(const item of history){const card=el('section');card.className='player-info-card';card.append(el('h3',item.bounty||'Bounty'),facts('Submission details',[['Game date',item.gameDate],['Submitted',item.submittedAt?new Date(item.submittedAt).toLocaleString():'Not submitted'],['Status',String(item.state||'Unknown').replace(/_/g,' ')],['Relationship',item.submittedByPlayer?'Submitted by this player':'Player recorded in screenshot'],['Screenshots',item.fileCount||0],['Bounty reward',`${item.rewardPoints??10} points`],['Points awarded',item.submittedByPlayer?(item.pointsAwarded??0):'Not awarded for being listed in evidence'],['Reviewed',item.reviewedAt?new Date(item.reviewedAt).toLocaleString():'Awaiting review']]));if(item.reviewNote)card.append(el('p',item.reviewNote));body.append(card);}return;
 }
