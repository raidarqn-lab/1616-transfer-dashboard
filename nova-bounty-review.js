...
function editPlayerOverview(body,profile,full,onSaved){
 const form=el('form');form.className='player-info-card hr-form';form.append(el('h3','Edit shared player details'));
 ...
 for(const [key,label] of fields){const wrap=el('label',label),input=el(key==='allianceRank'?'select':'input');if(key==='allianceRank'){for(const rank of ['','R1','R2','R3','R4','R5']){const option=el('option',rank||'Not recorded');option.value=rank;input.append(option);}}else{input.type=['power','profession','kills'].includes(key)?'number':'text';input.maxLength=160;if(input.type==='number'){input.min='0';input.max='1000000000000000';input.step=key==='power'?'any':'1';}}input.value=full.details?.[key]??'';input.required=['name','server'].includes(key);if(key==='server')input.pattern='[0-9]{1,6}';wrap.append(input);form.append(wrap);inputs.set(key,input);}
 const save=el('button','Review changes'),cancel=el('button','Cancel');cancel.type='button';cancel.onclick=()=>form.remove();const feedback=el('p');feedback.setAttribute('role','status');form.append(save,cancel,feedback);body.prepend(form);
 form.onsubmit=async event=>{event.preventDefault();const changes={},lines=[];for(const [key,label] of fields){const input=inputs.get(key),value=input.type==='number'?(input.value===''?null:Number(input.value)):input.value.trim(),old=full.details?.[key]??(input.type==='number'?null:'');if(String(value??'')!==String(old??'')){changes[key]=value;lines.push(`${label}: ${old??'Not recorded'} → ${value??'Not recorded'}`);}}
 if(!lines.length){feedback.textContent='No changes to save.';return;}
 if(!confirm('Save these changes to the shared player record used by both hubs?\n\n'+lines.join('\n')))return;
 save.disabled=true;cancel.disabled=true;feedback.textContent='Saving shared player record…';try{const saved=await call({action:'profile',playerKey:profile.key,profileOperation:'save',confirmed:true,expectedRevision:full.revision,changes,includeActivity:true});directoryCache.clear();profileCache.clear();onSaved({...profile,...saved.details});}catch(error){feedback.textContent='Not saved. If this player changed elsewhere, reopen the profile before trying again. '+error.message;}finally{save.disabled=false;cancel.disabled=false;}};
}
