...
function contentCard(item){const button=document.createElement('button');button.type='button';button.className='r4-queue-item';button.innerHTML=`<span>${escapeHtml(item.status)}</span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.alliance||'All Sapphire members')} · revision ${item.revision}</small>`;button.onclick=()=>fillContent(item);return button;}
...
async function saveContent(event){event.preventDefault();const form=event.currentTarget,kind=form.id==='event-editor'?'event':'announcement',values=Object.fromEntries(new FormData(form)),id=form.dataset.id||'',revision=Number(form.dataset.revision||0);const payload=kind==='event'?{category:values.category}:{priority:values.priority,audience:values.audience};try{await hubCall({action:'hub-content-save',id,revision,kind,status:values.status,alliance:'NvSP',title:values.title,body:values.body||'',startsAt:values.startsAt?`${values.startsAt}:00Z`:'',endsAt:values.endsAt?`${values.endsAt}:00Z`:'',payload});form.reset();delete form.dataset.id;delete form.dataset.revision;await loadContent();setStatus(`${kind==='event'?'Event':'Announcement'} saved. Published content is now available to signed-in members on nova.join1616.com.`);}catch(error){setStatus(error.message,true);}}
...
function paintEventType(){document.querySelectorAll('[data-event-type]').forEach(button=>{const selected=button.dataset.eventType===eventTypeSelect.value;button.classList.toggle('active',selected);button.setAttribute('aria-pressed',String(selected));});}
