...
async function saveContent(event){event.preventDefault();const form=event.currentTarget,kind=form.id==='event-editor'?'event':'announcement',values=Object.fromEntries(new FormData(form)),id=form.dataset.id||'',revision=Number(form.dataset.revision||0);const payload=kind==='event'?{category:values.category}:{priority:values.priority,audience:values.audience};try{await hubCall({action:'hub-content-save',id,revision,kind,status:values.status,alliance:'NvSP',title:values.title,body:values.body||'',startsAt:values.startsAt||'',endsAt:values.endsAt||'',payload});form.reset();delete form.dataset.id;delete form.dataset.revision;await loadContent();setStatus(`${kind==='event'?'Event':'Announcement'} saved. Published content is now available to signed-in members on nova.join1616.com.`);}catch(error){setStatus(error.message,true);}}
